<html>
		...
		if (empty($_SESSION['user_id'])) {
			...
			if ($access_level > 1) {
				...
				while ($row = mysqli_fetch_array($result)) {
					...
					<td><a href="delete_admin_users.php?id=' . $row['admin_id'] . '" onclick="return confirm(\'Are you want to delete this Administrator?\');">Delete</a></td></tr>';
				}
	...
	?>
		...
		<!--These buttons allow any user to go back to business contacts and only administrators with an access level greater than one to see these buttons-->
		<?php
		if ($access_level > 1) {
			echo "<br><a href=\"add_admin_users.php\"><button>Add New Administrator</button></a><a href=\"admin_users.php\"><button>Back to Business Contacts</button></a>";
		}
		?>
	</body>
</html>